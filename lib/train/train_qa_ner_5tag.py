...
from lib.data_prerocess.ner_base_5tag import load_data
...
def convert_answer_idx2text(cur_pre_ids, tokens):
  """
  tag 在 1,2,3 的字符皆认为是 answer。
  Args:
    cur_pre_ids:
  ...
  """
  return "".join([tokens[idx] for idx, i in enumerate(cur_pre_ids) if i in [1,2,3]]) if cur_pre_ids.tolist() else ""

def convert_answer_idx2text2(cur_pre_ids, tokens):
  """
  第一组 tag 1~3 且中间皆为 2 的 answer。
  Args:
    cur_pre_ids:
  ...
  for v_list in result:
    res = reduce(lambda x,y: x if len(x) >= len(y) else y, v_list) if v_list else ""
    if res:
      return res
  return ""
...
def convert_pre_res_binary_cls(input_ids, pre_ids, ori_start, ori_end, tokenizer):
  ...
  for input, cur_pre_ids, o_start, o_end in zip(input_ids, pre_ids, ori_start, ori_end):
    ...
    predict_answer = convert_answer_idx2text(cur_pre_ids, tokens)
    predict_answer2 = convert_answer_idx2text2(cur_pre_ids, tokens)
    cur_res = {
      "context": context,
      "question": question,
      "label_answer": label_answer,
      "predict_answer": predict_answer,
      "predict_answer2": predict_answer2,
